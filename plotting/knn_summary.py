"""KNN summary aggregation and visualization."""
...
def update_random_seed_summary(summary, result):
    """Aggregate KNN result into per-dataset summary."""
...
def finalize_random_seed_summary(summary):
    """Finalize KNN summary with mean/std across seeds."""
    finalized = {}
...
def save_random_seed_artifacts(model_name: str, summary):
    """Save KNN summary JSON and plot."""
    if not summary:
        return
    ...
    for idx, dataset_name in enumerate(datasets):
        ...
        # Prefer explicit R2 label for regression when present.
        if "roc_auc" in metric_name:
            metric_label = "Mean ROC-AUC"
        elif "r2" in metric_name:
            metric_label = "Mean R2"
        else:
    ...
    fig.suptitle(f"KNN Performance Across Runs for model: {title_model_name}{title_suffix}")
    fig.tight_layout()
    plot_path = model_dir / "knn_random_seed_summary.png"
    fig.savefig(plot_path, dpi=300)
    plt.close(fig)

    print(f"Saved KNN summary JSON: {json_path}")
    print(f"Saved KNN summary plot: {plot_path}")
